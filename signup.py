...
def signup(username : str, password : str, email : str):  
	users = getUsers()
	if(users != None):
		if(get_fb_instance().child("delay").get() != None):
			if(float(get_fb_instance().child("delay").get().val())+10 < float(time.time())):
				get_fb_instance().update({"delay" : str(time.time())})
			else:
				flash("Too many users are signing up right now! Please try again later.")
				return redirect(url_for("signup_pages.signup_page"))
		else:
			get_fb_instance().update({"delay" : str(time.time())})
	if users != None and [x for x in users if x.key().lower() == username.lower() or parseUser(x).email.lower() == email.lower()]:
