...
def sendResetEmail(email):
    ...
    if(len(user) > 0):
        ...
        UUID = str(uuid.uuid3(uuid.NAMESPACE_DNS, email)) + \
            str(uuid.uuid4()) + "--" + str(epoch + 900)
...
def reset_user(token):
    if(get_fb_instance().child("userdata/reset").get().each() is None):
        abort(403, "Token expired or does not exist")
    if(request.method == 'POST'):
        user = [x for x in get_fb_instance().child(
            "userdata/reset").get().each() if x.val() == str(token)]
        try:
        ...
        if(len(user) > 0 and float(epoch) > now):
            user = parseUser(
                [x for x in getUsers() if parseUser(x).id == user[0].key()][0])
