...
class User:
    def __init__(self, id: str, username: str, password: str, role: int, email : str, notes : str):
        self.id = id
...
def parseUser(data): # Smarter implementation
    if(isinstance(data, str)):
        ...
        return User(data["id"], data["username"], data["password"], data["role"], data["email"], data["notes"])
...
def deleteUser(data, *method):
    try:
        ...
        if(len(method) == 0 or method[0] == "username"):
            user = [x for x in users if parseUser(x).username == data or parseUser(x).email == data][0]
...
def updateUser(id, user_object):
    user = getUser(id)
    get_fb_instance().child("users").update({user.username : json.dumps(user_object.__dict__)})
